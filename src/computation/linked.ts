import { Template, AnonymousTemplate } from "./template";

type Order = Pointer | [Pointer, Pointer];

export class LinkedTemplate extends Template {
    ...
    static dereferencePointers(
        ...
        var references: { [index: string]: Template } = {};
    ...
    private iterateApplicationOrderDetermination(
        ...
        unappliedTemplates.forEach(
            template => {
                this.remove(unappliedTemplates, template);
                appliedTemplates.push(template);
                applicationOrder.push(template);
            }
        );
        unappliedLinks.forEach(
            link => {
                this.remove(unappliedLinks, link);
                appliedLinks.push(link);
                applicationOrder.push(link);
            }
        );
